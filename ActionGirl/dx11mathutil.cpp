...
ワールド変換行列を作る
--------------------------*/
void DX11MakeWorldMatrix(XMFLOAT4X4& inoutmat,
	XMFLOAT3& angle,
	XMFLOAT3& trans,
	float sizes)
{
	ALIGN16 XMMATRIX mat, matscale, matx, maty, matz, mattrans;
	matscale = XMMatrixScaling(sizes, sizes, sizes);
...
//void DX11MakeWorldMatrix(XMFLOAT4X4& inoutmat, XMFLOAT3& angle, XMFLOAT3& trans, float sizeX,float sizeY,float sizeZ)
//{
//	ALIGN16 XMMATRIX mat, matscale, matx, maty, matz, mattrans;
//	matscale = XMMatrixScaling(sizeX, sizeY, sizeZ);
//	matx = XMMatrixRotationX((angle.x*XM_PI) / 180.0f);
//	maty = XMMatrixRotationY((angle.y*XM_PI) / 180.0f);
//	matz = XMMatrixRotationZ((angle.z*XM_PI) / 180.0f);
//	mattrans = XMMatrixTranslation(trans.x, trans.y, trans.z);
...
void DX11MtxFromQt(XMFLOAT4X4& outmtx, const XMFLOAT4& qt){
	...
	ALIGN16 XMMATRIX mtx,mtxscale;
	ALIGN16 XMVECTOR qtvec;
	//mtxscale = XMMatrixScaling(size, size, size);
	qtvec = XMLoadFloat4(&qt);

	mtx = /*mtxscale * */XMMatrixRotationQuaternion(qtvec);
...
/*------------------------
  行列の転置
--------------------------*/
void DX11MtxTranspose(XMFLOAT4X4& mtx) {

	ALIGN16 XMMATRIX mtxin;
	ALIGN16 XMMATRIX mtxout;

	mtxin = XMLoadFloat4x4(&mtx);

	mtxout = XMMatrixTranspose(mtxin);

	XMStoreFloat4x4(&mtx, mtxout);
}
